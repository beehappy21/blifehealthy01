...
type CartItem = {
  ...
  productId: string;
  name: string;
  selected: { size?: string; color?: string; pack?: string };
  price: number;
  pv: number;
  qty: number;
};
...
export default function CartPage() {
  ...
  const inc = (key: string) => save(cart.map((x) => (x.key === key ? { ...x, qty: x.qty + 1 } : x)));
  const dec = (key: string) => save(cart.map((x) => (x.key === key ? { ...x, qty: Math.max(1, x.qty - 1) } : x)));
              ...
              <div key={item.key} style={{ border: "1px solid #eee", borderRadius: 12, padding: 12, display: "flex", justifyContent: "space-between", gap: 12 }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 13, color: "#666", marginTop: 4 }}>
                    ตัวเลือก:{" "}
                    {[
                      item.selected.size && `Size:${item.selected.size}`,
                      item.selected.color && `Color:${item.selected.color}`,
                      item.selected.pack && `Pack:${item.selected.pack}`,
                    ]
                      .filter(Boolean)
                      .join(" / ") || "-"}
                  </div>
                ...
                <div style={{ display: "grid", gap: 8, alignContent: "start" }}>
                  <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    ...
                    <b>{item.qty}</b>
                    <button onClick={() => inc(item.key)}>+</button>
                  </div>
